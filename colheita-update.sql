...
DELETE FROM `mydb`.`itens_pedido`
WHERE `id_produto` IN (SELECT id FROM `mydb`.`produtos` WHERE `categorias_id` = (SELECT id FROM `mydb`.`categorias` WHERE nome = 'Cozinha'));

DELETE FROM `mydb`.`produtos`
WHERE `categorias_id` = (SELECT id FROM `mydb`.`categorias` WHERE nome = 'Cozinha');
