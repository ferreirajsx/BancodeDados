...
ADD COLUMN `observacoes` VARCHAR(255) NULL AFTER `pedido`;

ALTER TABLE `mydb`.`Usuarios`
ADD COLUMN `data_nascimento` DATE NULL AFTER `Data_cadastro`;

ALTER TABLE `mydb`.`Estabelecimentos`
MODIFY COLUMN `Telefone` VARCHAR(60) NULL;

ALTER TABLE `mydb`.`produtos`
ADD COLUMN `estoque_minimo` INT NOT NULL DEFAULT 0 AFTER `quantidade`;

ALTER TABLE `mydb`.`enderecos`
ADD INDEX `idx_cep` (`cep` ASC) VISIBLE;

ALTER TABLE `mydb`.`categorias`
ADD COLUMN `ultima_atualizacao` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER `nome`;

ALTER TABLE `mydb`.`produtos`
MODIFY COLUMN `descricao` VARCHAR(500) NOT NULL;

ALTER TABLE `mydb`.`Estabelecimentos`
ADD COLUMN `data_abertura` DATE NULL AFTER `endereco`;

ALTER TABLE `mydb`.`Usuarios`
ADD COLUMN `pontuacao_lealdade` INT NOT NULL DEFAULT 0 AFTER `Email`;

ALTER TABLE `mydb`.`itens_pedido`
ADD COLUMN `desconto_aplicado` DECIMAL(5,2) NOT NULL DEFAULT 0.00 AFTER `valor`;
...
UPDATE `mydb`.`produtos`
SET `preco` = 7.50
WHERE `id` = 1;

UPDATE `mydb`.`Usuarios`
SET `Email` = 'joao.silva.novo@gmail.com'
WHERE `id` = 1;

UPDATE `mydb`.`Estabelecimentos`
SET `Telefone` = '(11) 98765-4321'
WHERE `id` = 2;

UPDATE `mydb`.`enderecos`
SET `complemento` = 'Apartamento 10'
WHERE `id` = 1;
...
UPDATE `mydb`.`produtos`
SET `descricao` = 'Massa de trigo premium tipo espaguete'
WHERE `id` = 1;

UPDATE `mydb`.`pedidos`
SET `status` = 'Finalizado'
WHERE `id_estabelecimento` = 1 AND `status` = 'Entregue';

UPDATE `mydb`.`produtos`
SET `preco` = `preco` * 1.10
WHERE `categorias_id` = (SELECT id FROM `mydb`.`categorias` WHERE nome = 'Bebidas');

UPDATE `mydb`.`Usuarios`
SET `pontuacao_lealdade` = 15
WHERE `id` >= 15 ;

#---------------------------- divisoria ---------------------------- #

DELETE FROM `mydb`.`itens_pedido`
WHERE `id` = 17;

DELETE FROM `mydb`.`produtos`
WHERE `id` = 18;

DELETE FROM `mydb`.`pedidos`
WHERE `id` = 5;

DELETE FROM `mydb`.`enderecos`
WHERE `Usuarios_id` = 67;

DELETE FROM `mydb`.`pedidos`
WHERE `id_usuario` = 67;

DELETE FROM `mydb`.`Usuarios`
WHERE `id` = 67;

SET SQL_SAFE_UPDATES = 0;

DELETE FROM `mydb`.`pedidos`
WHERE `data_pedido` < '2025-07-01 10:30:00';

DELETE FROM `mydb`.`itens_pedido`
WHERE `id_produto` IN (SELECT id FROM `mydb`.`produtos` WHERE `categorias_id` = (SELECT id FROM `mydb`.`categorias` WHERE nome = 'Mercearia'));

DELETE FROM `mydb`.`produtos`
WHERE `categorias_id` = (SELECT id FROM `mydb`.`categorias` WHERE nome = 'Mercearia');

DELETE FROM `mydb`.`categorias`
WHERE `nome` = 'Bebidas';

DELETE FROM `mydb`.`Estabelecimentos`
WHERE `id` = 15;
...
DELETE FROM `mydb`.`enderecos`
WHERE `complemento` IS NULL AND `id` > 10;


#------- Perigoso esse daqui -------#
DELETE FROM `mydb`.`Usuarios`
WHERE `Tipo` = 'fornecedor' AND `id` NOT IN (SELECT `id_usuario` FROM `mydb`.`Estabelecimentos` WHERE `id_usuario` IS NOT NULL);
